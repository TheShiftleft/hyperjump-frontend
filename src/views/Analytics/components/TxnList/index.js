...
import { formatTime, formattedNum, urls } from '../../../../utils/analytics'
import { useCurrentCurrency } from '../../../../contexts/Analytics/Application'
...
import { Divider, EmptyCard, TYPE } from '..'
...
const SortText = styled.button`
  ...
  font-size: 1rem;
  padding: 0px;
  color: ${({ active, theme }) => (active ? theme.colors.text1 : theme.colors.text3)};
  outline: none;
...
function TxnList({ transactions, symbol0Override, symbol1Override, color }) {
    ...
    return (
      <DashGrid style={{ height: '48px' }}>
        <DataText area="txn" fontWeight="500">
          <Link color={({ theme }) => theme.colors.blue} external href={urls.showTransaction(item.hash)}>
        ...
        {!below780 && (
          <>
            <DataText area="amountOther">
              ...
              <FormattedName text={item.token1Symbol} maxCharacters={5} margin  />
            </DataText>
            <DataText area="amountToken">
              {`${formattedNum(item.token0Amount)} `}{' '}
              <FormattedName text={item.token0Symbol} maxCharacters={5} margin  />
            </DataText>
          </>
        )}
        {!below1080 && (
  ...
  return (
    <>
      <DashGrid center  style={{ height: 'fit-content', padding: '0 0 1rem 0' }}>
        {below780 ? (
          <RowBetween area="txn">
