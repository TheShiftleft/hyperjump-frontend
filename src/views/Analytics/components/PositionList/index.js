...
import { formattedNum, getPoolLink } from '../../../../utils/analytics'
...
function PositionList({ positions }) {
  ...
  const ListItem = ({ position, index }) => {
    ...
    return (
      <DashGrid style={{ opacity: poolOwnership > 0 ? 1 : 0.6 }} focus >
        ...
        <DataText area="name" justifyContent="flex-start" alignItems="flex-start">
          ...
          <AutoColumn gap="8px" justify="flex-start" style={{ marginLeft: '20px' }}>
            <CustomLink to={`/analytics/pair/${position.pair.id}`}>
              <TYPE.main style={{ whiteSpace: 'nowrap' }} to='/analytics/pair/'>
                <FormattedName
  ...
  return (
    <ListWrapper>
      <DashGrid center  style={{ height: '32px', padding: 0 }}>
        {!below740 && (
