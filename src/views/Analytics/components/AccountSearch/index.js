...
import { useSavedAccounts } from '../../../../contexts/Analytics/LocalStorage'
import { AutoColumn } from '../../../../components/Column'
import { TYPE, Hover, StyledIcon, Divider } from '..'
...
function AccountSearch({ history, small }) {
  ...
  function handleAccountSearch() {
    if (isAddress(accountValue)) {
      history.push(`/analytics/account/${accountValue}`)
          ...
          <Panel>
            ...
            {savedAccounts?.length > 0 ? (
              savedAccounts.map(account => {
                return (
                  <DashGrid key={account} center style={{ height: 'fit-content', padding: '1rem 0 0 0' }}>
                    <Flex
                      area="account"
                      justifyContent="space-between"
                      onClick={() => history.push(`/analytics/account/${account}`)}
                    >
              ...
              savedAccounts.map(account => {
                return (
                  <RowBetween key={account}>
                    <ButtonFaded onClick={() => history.push(`/analytics/account/${account}`)}>
                      {small ? (
