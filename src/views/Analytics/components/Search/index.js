...
import { useAllTokenData, useTokenData } from '../../../../contexts/Analytics/TokenData'
import { useAllPairData, usePairData } from '../../../../contexts/Analytics/PairData'
import DoubleTokenLogo from '../DoubleLogo'
import { useAllPairsInStreetswap, useAllTokensInStreetswap } from '../../../../contexts/Analytics/GlobalData'
...
const Wrapper = styled.div`
  ...
  @media screen and (max-width: 500px) {
    background: ${({ theme }) => transparentize(0.4, theme.colors.bg1)};
    box-shadow: ${({ open }) =>
      !open
        ? '0px 24px 32px rgba(0, 0, 0, 0.04), 0px 16px 24px rgba(0, 0, 0, 0.04), 0px 4px 8px rgba(0, 0, 0, 0.04), 0px 0px 1px rgba(0, 0, 0, 0.04) '
        : 'none'};
        ...
        <div>
          ...
          {filteredPairList &&
            filteredPairList.slice(0, pairsShown).map(pair => {
              ...
              return (
                <BasicLink to={`/analytics/pair/${pair.id}`} key={pair.id} onClick={onDismiss}>
                  <MenuItem>
                    <DoubleTokenLogo a0={pair?.token0?.id} a1={pair?.token1?.id} margin  />
