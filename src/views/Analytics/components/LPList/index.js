...
function LPList({ lps, disbaleLinks, maxItems = 10 }) {
    ...
    return (
      <DashGrid style={{ height: '48px' }} disbaleLinks={disbaleLinks} focus >
        ...
        <DataText area="name" fontWeight="500" justifyContent="flex-start">
          <CustomLink style={{ marginLeft: below600 ? 0 : '1rem', whiteSpace: 'nowrap' }} to={`/analytics/account/${lp.user.id}`}>
        ...
        <DataText>
          <CustomLink area="pair" to={`/analytics/pair/${lp.pairAddress}`}>
            <RowFixed>
              {!below600 && <DoubleTokenLogo a0={lp.token0} a1={lp.token1} size={16} margin  />}
              {lp.pairName}
            </RowFixed>
          </CustomLink>
        </DataText>
  ...
  return (
    <ListWrapper>
      <DashGrid center  disbaleLinks={disbaleLinks} style={{ height: 'fit-content', padding: ' 0 0 1rem 0' }}>
        {!below600 && (
