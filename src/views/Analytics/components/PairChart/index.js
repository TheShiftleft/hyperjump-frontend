...
import { toK, toNiceDate, toNiceDateYear, formattedNum, getTimeframe } from '../../../../utils/analytics'
import { OptionButton } from '../ButtonStyled'
import { usePairChartData, useHourlyRateData, usePairData } from '../../../../contexts/Analytics/PairData'
import { timeframeOptions } from '../../../../config/constants/analytics'
...
const PairChart = ({ address, color, base0, base1 }) => {
  ...
  const valueFormatter = (val) => {
    if (chartFilter === CHART_VIEW.RATE0) {
      return (
        `${formattedNum(val)}<span style="font-size: 12px; margin-left: 4px;">${formattedSymbol1}/${formattedSymbol0}<span>`
      )
    }
    if (chartFilter === CHART_VIEW.RATE1) {
      return (
        `${formattedNum(val)}<span style="font-size: 12px; margin-left: 4px;">${formattedSymbol0}/${formattedSymbol1}<span>`
      )
    }
    return '';
  }
