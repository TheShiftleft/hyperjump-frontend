...
import { PageWrapper, ContentWrapperLarge, StyledIcon, Hover, TYPE, ThemedBackground } from '../components'
...
import { formattedNum, formattedPercent, getPoolLink, getAstroLink, getSwapLink } from '../../../utils/analytics'
import { useColor } from '../../../hooks'
import { usePairData, usePairTransactions } from '../../../contexts/Analytics/PairData'
import CopyHelper from '../components/Copy'
import DoubleTokenLogo from '../components/DoubleLogo'
import TokenLogo from '../components/TokenLogo'
import { useBnbPrice } from '../../../contexts/Analytics/GlobalData'
import Warning from '../components/Warning'
import { usePathDismissed, useSavedPairs } from '../../../contexts/Analytics/LocalStorage'

import FormattedName from '../components/FormattedName'
import { useListedTokens } from '../../../contexts/Analytics/Application'
    ...
    <PageWrapper>
      ...
      <ContentWrapperLarge>
        ...
        <AutoRow align="flex-end" style={{ width: 'fit-content' }}>
          <TYPE.body>
            <BasicLink to="/analytics/pairs">{'Pairs '}</BasicLink>→ {token0?.symbol}-{token1?.symbol}
          </TYPE.body>
          <Link
        ...
        >
          <DashboardWrapper>
            <AutoColumn gap="40px" style={{ marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', width: '100%' }}>
                <RowFixed style={{ flexWrap: 'wrap', minWidth: '100px' }}>
                  <RowFixed>
                    {token0 && token1 && (
                      <DoubleTokenLogo a0={token0?.id || ''} a1={token1?.id || ''} size={32} margin  />
                    )}{' '}
                    <TYPE.main fontSize={below1080 ? '1.5rem' : '2rem'} style={{ margin: '0 1rem' }}>
                      {token0 && token1 ? (
                        <>
                          <HoverSpan onClick={() => history.push(`/analytics/token/${token0?.id}`)}>{token0.symbol}</HoverSpan>
                          <span>-</span>
                          <HoverSpan onClick={() => history.push(`/analytics/token/${token1?.id}`)}>
                            {token1.symbol}
                          </HoverSpan>{' '}
                          Pair
                        </>
            ...
            >
              <FixedPanel onClick={() => history.push(`/analytics/token/${token0?.id}`)}>
                <RowFixed>
              ...
              <FixedPanel onClick={() => history.push(`/analytics/token/${token1?.id}`)}>
                <RowFixed>
              ...
              <PanelWrapper style={{ marginTop: '1.5rem' }}>
                ...
                <Panel style={{ height: '100%' }}>
                  <AutoColumn gap="20px">
                    ...
                    <Hover onClick={() => history.push(`/analytics/token/${token0?.id}`)} fade>
                      <AutoRow gap="4px">
                        <TokenLogo address={token0?.id} />
                        <TYPE.main fontSize={20} lineHeight={1} fontWeight={500}>
                          <RowFixed>
                            {reserve0 ? formattedNum(reserve0) : ''}{' '}
                            <FormattedName text={token0?.symbol ?? ''} maxCharacters={8} margin />
                          </RowFixed>
                        </TYPE.main>
                      </AutoRow>
                    </Hover>
                    <Hover onClick={() => history.push(`/analytics/token/${token1?.id}`)} fade>
                      <AutoRow gap="4px">
                        <TokenLogo address={token1?.id} />
                        <TYPE.main fontSize={20} lineHeight={1} fontWeight={500}>
                          <RowFixed>
                            {reserve1 ? formattedNum(reserve1) : ''}{' '}
                            <FormattedName text={token1?.symbol ?? ''} maxCharacters={8} margin />
                          </RowFixed>
                        </TYPE.main>
                      </AutoRow>
                    </Hover>
