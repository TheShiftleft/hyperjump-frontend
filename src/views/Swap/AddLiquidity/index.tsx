...
const routerMethods: Record<Network, string> = {
...
const AddLiquidity = ({
  ...
  const [approvalA, approveACallback] = useApproveCallback(parsedAmounts[Field.CURRENCY_A], getRouterAddress())
  const [approvalB, approveBCallback] = useApproveCallback(parsedAmounts[Field.CURRENCY_B], getRouterAddress())
  ...
  async function onAdd() {
    ...
    if (currencyA === config.baseCurrency || currencyB === config.baseCurrency) {
      const tokenBIsETH = currencyB === config.baseCurrency
      estimate = router.estimateGas[routerMethods[config.network]]
      method = router[routerMethods[config.network]]
      args = [
  ...
  const handleCurrencyBSelect = useCallback(
    (currB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        ...
        history.push(`/add/${currencyIdA || config.networkToken.symbol}/${newCurrencyIdB}`)
      }
    },
    [currencyIdA, currencyIdB, history, config.networkToken.symbol],
  )
