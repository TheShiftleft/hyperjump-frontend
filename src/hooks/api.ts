...
export const useApprovedTransaction = () => {
  ...
  const query =
    chainId === 250
      ? `https://api.ftmscan.com/api?module=account&action=txlist&address=${account}`
      : `https://api.bscscan.com/api?module=account&action=txlist&address=${account}`
            ...
            if (Array.isArray(prev)) {
              if (current.allowanceAmount === 0) {
                const newArray = prev.filter(
                  (element) => !(element.approved === current.approved && element.contract === current.contract),
                )
                return newArray
              }
              return [...prev, current]
