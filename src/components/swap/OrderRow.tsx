...
export default function OrderRow({ order, account, chainId }: OrderRowProps) {
  ...
  const transactionURL =
    config.network === 'FTM'
      ...
      : `${BASE_BSC_SCAN_URL}/tx/${order?.createdTxHash}`
  ...
  return (
    <Row>
      <RowContainer>
        <TokenContainer>
          <Text fontSize="14px" bold>
            {order.inputToken === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
              ? config.networkToken.symbol
              : inputToken?.symbol}
          </Text>
          <ArrowForwardIcon color="primary" marginLeft="10px" marginRight="10px" width="24px" />
          <Text fontSize="14px" bold>
            {outputToken?.symbol}
          </Text>
        </TokenContainer>
      ...
      {detailsToggle && (
        <DetailContainer>
          <DetailsText>
            Date Created: <DetailsTextContent>{createdDate}</DetailsTextContent>
          </DetailsText>
          <DetailsText>
            Amount: <DetailsTextContent>{parsedInputAmount}</DetailsTextContent>
          </DetailsText>
          <DetailsText>
