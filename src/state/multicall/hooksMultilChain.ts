...
function useCallsData(multChainId: number ,calls: (Call | undefined)[], options?: ListenerOptions): CallResult[] {
  const chainId = multChainId
...
export function useMultiChainContractSingleData(
  multChainId: number,
...
export function useSingleCallResult(
  ...
  const result = useCallsData(multChainId, calls, options)[0]
  const latestBlockNumber = useBlockNumber()
